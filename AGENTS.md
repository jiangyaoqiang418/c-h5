...
- 后台协作、审核、测试数据准备和后台页面回读统一使用已部署入口 `https://testhou.merchantsale.store/`；不为这类工作启动或访问本地后台管理服务。`localhost:5174` 仅用于运行 C-H5 本身的本地开发页面和代理验证。
- 后台登录仅使用后台项目文档中明确记录的开发、测试或演示账号；不在 C-H5 文档、代码、日志或 Git 中复制账号、密码、Token 等凭据。
...
- 后台测试账号的权威来源是后台项目文档；C-H5 文档只记录验证结论和所需角色，不记录凭据。
