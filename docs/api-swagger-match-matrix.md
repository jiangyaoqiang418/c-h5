...
|---|---|---|---|
| KYC 类型切换与创建恢复（未验收完） | 本批递归核对 user 的 submit/detail/files/upload/access；文件归本人且私有，详情证件号仅返回前 3 后 4 脱敏值，审核中/有效期内不可重提、过期可重提，无客户端创建幂等键 | 按账号持久原摘要/文件 ID/前次申请标识，不保存完整证件号或签名 URL；未知只核对不重发，原 ID/脱敏字段/文件关联与新提交版本校验，驳回/过期后主动重新填写；证件类型切换清旧图、上传类型代次隔离、表单互斥 | 类型切换 7 组源码断言通过；创建恢复已接入但尚缺存储故障、旧审核版本等专项验证与真实创建/审核/驳回重提，不计完整验收。私有影像资源地址问题仍保留后端边界 |
...
| 订单详情与物流表单 | 复用既有订单详情/组付款/取消/收货、物流详情/轨迹创建/异常标记，无接口改动或新增实时契约证据 | 页面/会话/弹层代次保护，写前 ID/角色/状态校验；失败读取保留数据、明确重试；成功及未知物流结果独立回执，轨迹精确 ID 或原请求组合回读，异常目标状态核对 | 生产源码异常分支断言与类型/H5/App 检查通过；Chrome 访客入口读取；真实物流写入、双端回读、跨入口/跨刷新保护仍待完整验收 |
