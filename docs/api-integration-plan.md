...
- 地址真实类型、API adapter 与地址页读取/新增/设默认/删除已经迁移；2026-08-07 使用本地受保护测试账号只读调用 `GET /user/addresses/list`，登录与地址响应成功码均为 `1`，当前列表为 0 条，原空态可继续使用。未执行任何地址写入，因此新增、设默认、删除及非空字段回显仍不得标记为真实验证完成。
