...
- 2026-08-05：`admin` 为 84 路径/85 操作/138 schema，`user` 为 19/19/45，`order` 为 40/42/50；三个文档版本均为 `v1.0.0`，`notify` 仍为 HTTP 404。
...
| 2 | 打通买手申请 | API、类型、页面、路由和入口已接入 | 待无申请/驳回账号验证真实提交；审核中账号验证只读状态 |
...
| 7 | 接入积分申诉记录 | API 和积分页“申诉记录”页签已接入 | 待有申诉数据账号验证状态、审核意见和时间回显 |
| 8 | 订单契约专项确认 | 逐字段清单已完成，见 `docs/order-contract-confirmation.md` | 待后端确认并更新 Swagger 后再迁移订单、结算和售后页面 |
...
- 已新增 `docs/order-contract-confirmation.md`，按地址、多商品、金额、状态、物流、取消/退款、Long ID 和实施门禁整理为可直接交给后端的确认清单。后端确认并更新 Swagger 前，订单、结算和售后页面继续保留 Mock，不提前迁移。
...
2026-08-05 的 P2 后续任务 2-7 已完成静态接入，下一步是使用具备对应数据和权限的账号做真实回归；订单模块继续等待下方契约确认。
...
1. 地址与结算：补充收货地址 CRUD/默认地址；明确下单传 `addressId` 还是收件人、手机号、完整地址快照。当前 `OrderCreateQO` 只有 `productId/quantity/sessionId/remark`。
2. 多商品策略：当前购物车可多商品结算，后端下单一次只接收一个 `productId`；需确认拆成多个订单还是新增批量下单契约。
3. 金额口径：明确并返回商品小计、运费、税费、优惠、改价前金额、实付金额和退款金额；当前 `OrderDTO` 只有 `originalAmount/unitPrice/totalAmount`，不足以还原结算页和详情页。
4. 物流契约：发货接口需确认承运商、运单号、发货凭证和预计到达时间的提交方式；订单详情需返回对应字段和物流轨迹。当前 `POST /orders/ship` 只接收订单 ID。
...
7. 订单快照：详情需确认买手名称、收货信息、售后配置、采购/发货凭证、取消原因与操作者、各状态时间和关联售后单 ID，避免页面从 Mock 拼接真实接口缺失字段。
