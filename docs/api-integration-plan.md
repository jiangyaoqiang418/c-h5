...
- **状态：部分完成。** 当前使用 `main` 分支，准确提交位置以 Git HEAD 为准；H5 真实接口主链已覆盖账户、商品、订单、钱包、评价、理财及通知读取，下一阶段继续收口 P3/P4 实时通信、受控写操作和三端里程碑验收。
- **最近完成：** 评价已完成待评价读取、五星提交和“我发出的”回读；理财已完成产品读取、`100 U` 申购、锁仓回读及通知跳转；公开商品、订单主链、资金流水、系统通知和订单群历史消息已有非空真实数据证据；IM WebSocket 客户端已实现连接、`READY` 门禁、心跳、401 停止重连和指数退避。
- **最近验证：** 2026-08-17 已通过 `pnpm typecheck`、`pnpm build:h5` 和 Chrome `393×852` 登录态回归；2026-08-20 仅完成文档清理与规则更新，未重复执行代码构建和浏览器回归。
- **未完成：** WebSocket 实际 `101/READY`、可控实时推送和文字发送尚未形成最终证据，图片/语音仍缺可确认的上传契约；评价删除、回复、申诉未执行真实写操作；理财提前赎回、提现创建及充值申报会影响测试资金或记录，未经即时授权不得执行；Android/iOS 真机回归留到模块里程碑集中完成。
- **已知阻塞：** 充值创建曾在 `TRON/ETH/BSC` 三条链均失败，具体业务原因仍待后端确认；买手申请“驳回后重提”缺可操作的后台审核入口和安全测试数据；涉及网关 WebSocket Upgrade、文件上传或后台页面异常时，由对应服务先提供可用环境，前端不使用 Mock 掩盖。
- **下一步：** 先刷新 live `swagger-config` 及目标分组契约；优先完成 WebSocket 握手、`READY` 和可控新消息到达回归，再按授权分别验证文字发送、评价治理写操作及资金类操作；状态变化后同步更新本计划和 Swagger 匹配矩阵。
- **换机启动：** 执行 `git status -sb`、`git fetch origin`、`git pull --ff-only`，读取 `AGENTS.md`、`README.md`、本计划和 Swagger 匹配矩阵；仅在依赖缺失或锁文件变化时运行 `pnpm install`。重新配置本机环境变量和 Git 忽略的 `.h5-test-account.local`，复用测试账号与 Chrome 登录态后继续。
