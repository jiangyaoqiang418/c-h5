...
- **状态：部分完成。** 当前使用 `main` 分支，准确提交位置以 Git HEAD 为准；H5 真实接口主链已覆盖账户、商品、订单、钱包、评价、理财、KYC 状态和通知/IM REST，下一阶段继续收口 WebSocket 实时到达、受控写操作与三端里程碑验收。
- **最近完成：** 系统通知页已注册并支持已读/删除/清空；IM 已接入会话、历史消息、文字发送、失败重试与 WebSocket 降级，修复乐观消息滚动锚点；KYC 已移除 Mock 资料和假提交，改为真实状态读取并明确阻断无上传契约的提交；充值页已按实时开放链和最低金额约束申报；理财申购/赎回、充值申报和提现补齐二次确认、提交拦截与回读。
- **最近验证：** 2026-08-20 已通过 `pnpm typecheck`、`pnpm build:h5`、`git diff --check` 和 Chrome 登录态回归。KYC `PASSED`、非空系统通知与订单群、待评价订单、理财/锁仓空态、充值开放链/专属地址均真实回读；单条通知已读由 `25` 回读为 `24`，QA 订单群文字发送后刷新回读成功。新浏览器会话无新增 error，仅有项目已有 `vue-router` 弃用 warning。
- **未完成：** WebSocket 实际 `101/READY` 与可控实时推送到达仍无证据；图片/语音和 KYC 提交仍缺明确 C 端上传契约；评价删除、回复、申诉，通知删除/清空尚未真实执行；理财提前赎回、提现创建和充值申报仍未验证；Android/iOS 真机回归留到模块里程碑集中完成。
- **已知阻塞：** 测试网关 `onlineConnections=0`，需网关/nginx 恢复 WebSocket Upgrade；充值创建曾在 `TRON/ETH/BSC` 三条链失败，具体业务原因待后端确认；买手申请“驳回后重提”缺可操作后台审核入口和安全测试数据；CMS、AI、买手经营/押金仍缺 C 端接口或关键字段。
- **下一步：** 网关恢复后优先验证 `101/READY`、心跳与无刷新新消息；随后使用安全测试数据完成评价治理及通知删除/清空，补齐上传契约后开放 KYC/IM 媒体；最后集中执行 Android/iOS 真机回归。
- **换机启动：** 执行 `git status -sb`、`git fetch origin`、`git pull --ff-only`，读取 `AGENTS.md`、`README.md`、本计划和 Swagger 匹配矩阵；仅在依赖缺失或锁文件变化时运行 `pnpm install`。使用项目明确记录的测试账号并复用 Chrome 登录态后继续，凭据不输出、不写入代码或日志。
...
- 评价已新增真实 `src/service/api/review.ts` 和类型：待评价订单、提交、我的/收到评价、商品评价分页、商品评分摘要、买手评分摘要，以及删除、回复、申诉。`pages/review/list` 和 `write` 已使用真实接口并提供删除、回复与申诉入口，操作具备加载、失败提示和成功后回读；订单列表/详情的“写评价”与订单详情“三方群”已改为真实跳转，无法评价的已完成订单明确展示空态；商品详情的真实商品分支已读取评分摘要和评价列表。
...
- 已只读验证 `GET /notify/back/im/status` 返回成功：网关 WebSocket 路径为 `/notify/im`，服务端心跳周期 `25000ms`。2026-08-20 登录态 Chrome 复核中，REST 消息中心正常回读，但服务端 `onlineConnections` 持续为 `0`，测试网关仍未完成 WebSocket Upgrade/`READY`；该阻塞与 c-pc 当前测试结果一致，需网关/nginx 恢复 Upgrade 透传后复验。
- `src/service/im-socket.ts` 已按服务约定以 `gatewayPath + token query` 组装 ws/wss 地址，只在收到 `READY` 后标记成功；发送 JSON `PING`、忽略 `PONG`，10 秒未收到 `READY` 主动断开，并保留 401 停止重连和上限 30 秒指数退避。H5 的 notify Vite 代理已开启 `ws: true`；订单群页会展示连接状态和手动重连入口。
- Chrome 的 iPhone 15 Pro 尺寸回归（`393×852`）已使用顾客测试账号完成：`/pages/review/list` 实际回读 3 笔待评价订单，第一笔可进入真实写评价页并带入订单与买手快照；我的/收到评价均正确回读为空态。`/pages/message/index` 回读 14 条系统未读、17 条 IM 未读及真实订单群预览，系统通知列表回读实际订单与货款通知。2026-08-20 已补注册系统通知页路由，并接入单条/全部已读、删除和清空操作；Chrome 实际回读 25 条未读通知和全部操作入口，未确认删除或清空，真实写入待安全测试数据确认。充值页回读 TRC20 专属地址及最低充值 `10 USDT`；理财产品与锁仓订单当前均为空，页面正确展示空态且无控制台错误。
- 2026-08-20 已按 live `user` Swagger 复核 `FinanceSubscribeQO`、`FinanceRedeemQO`、`RechargeCreateQO`、`WithdrawCreateQO` 的必填字段与金额约束。理财申购、提前赎回、充值申报和提现均补齐二次确认、提交中拦截、失败提示和成功后的余额或订单回读；Chrome 仅只读验证理财产品/锁仓空态与 TRC20 专属地址，不执行任何资金或锁仓写操作。
- 同日已接入 `GET /user/recharge/chains`：充值页只展示后端当前 `enabled` 的链，使用其 `chain/label/minAmount` 约束创建申报，并在链列表失败或为空时停止创建、不回退硬编码链。Chrome 登录态实际回读“波场”、最低 `10 USDT` 与专属地址；未创建充值申报单。
- KYC 页面已移除 Mock 状态、占位图片上传和假提交，改为调用 `GET /user/kyc/detail` 展示脱敏证件信息、状态、审核意见和时间；`POST /user/kyc/submit` 的类型与 API 已封装，但没有已确认的 C 端 KYC 文件上传契约，页面明确阻断在线提交。2026-08-20 Chrome 测试账号实际回读 `PASSED`，正确展示已通过状态、脱敏证件号、审核意见“同意”及提交/审核/有效期时间，页面无新增控制台错误；真实提交仍未执行。
- 2026-08-20 登录态全链路回归：消息中心回读系统未读 `25`、IM 未读 `23` 和最新预览；系统通知页非空记录完整展示。点击一条理财赎回通知后跳转锁仓页，未读数由 `25` 回读为 `24`。订单群、待评价订单、理财产品/锁仓空态和充值开放链/专属地址均正常回读。
...
- 订单群页现自行启动并订阅 IM WebSocket，离开页面时释放订阅和连接；`imSocket.start` 已改为幂等，避免消息中心与群页切换时重复建连。文字发送已调用真实 `POST /notify/im/messages/send`，使用 `clientMsgId` 乐观上屏，成功后重新读取会话，失败保留“点击重试”。2026-08-20 已在 QA 订单群发送测试文字并刷新回读；同时修复乐观消息 `local:<id>` 含冒号被 H5 `scroll-into-view` 作为 CSS 选择器解析的错误，锚点统一安全化。新 Chrome 会话复验文字发送无 error。服务端恢复 `101/READY` 后，仍需用可控新消息事件完成无刷新到达验证。
- 评价非空读链路补测：在 iPhone 15 Pro 尺寸切换至“我收到的”，实际读取两条已发布五星评价，其中一条正确展示买手回复；“我发出的”在当前会话为空态，两个页签均无控制台错误。2026-08-20 Chrome 再次验证评价页签空态及订单“写评价”真实跳转，对已评价订单正确显示“当前订单暂不可评价”；删除、回复、申诉仍未执行写操作。
