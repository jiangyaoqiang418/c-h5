...
- 真实订单已建立买入/卖出分页和详情只读类型与 API adapter；订单页面仍保留 Mock，等待后端 7 状态与现有前端 10 状态的产品映射确认后再迁移。
...
2026-07-28 已完成历史 Mock 能力扫描。2026-08-10 已将实时 Swagger 原始快照保存到 `docs/swagger-baselines/2026-08-10/`，并新增 `pnpm swagger:check` 递归比较命令；当前结果为：`admin` 107 路径/108 操作/187 schema，`user` 33/33/66，`order` 43/45/63，`notify` 9/9/18。相较 2026-08-09，admin/order 无递归差异；user 新增仅开发测试使用的充值确认契约，notify 从 HTTP 404 恢复为可读文档，但 IM/通知仍属 P3，不在本轮接入。真实模块迁移前必须完成最新递归差异审查：
...
- 2026-08-09 只读实测：登录、当前用户、积分、钱包、分类、地址、买入/卖出订单和公开商品接口均返回成功码 `1`；当前测试账号为 `CUSTOMER/BUYER`、KYC 为 `UNSUBMITTED`，分类为 5 个根节点、地址为 1 条，公开商品与买入/卖出订单均为空。以上仅表示服务可用或空态可验证，不表示订单闭环已完成。
