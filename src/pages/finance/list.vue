...
async function load() {
  ...
  try {
    ...
    uni.showToast({ title: error instanceof Error ? error.message : '小金库加载失败', icon: 'none' });
  } finally {
    loading.value = false;
  }
}
