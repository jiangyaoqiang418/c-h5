...
onLoad(async query => { const id = String(query?.id || ''); if (!id) { loading.value = false; return; } try { await userStore.init(); if (!userStore.currentUser) { await requireLogin(`/pages/finance/detail?id=${encodeURIComponent(id)}`); return; } await load(id); } catch (error) { loadFailed.value = true; loading.value = false; uni.showToast({ title: error instanceof Error ? error.message : '理财产品加载失败', icon: 'none' }); } });
