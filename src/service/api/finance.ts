...
export function fetchFinanceOrderByKey(idempotencyKey: string) { return realUserRequest<Api.RealFinance.OrderVO | null>({ url: '/finance/orders/by-key', params: { idempotencyKey }, requireDataEnvelope: true }); }
...
export function redeemFinanceOrder(id: Api.RealFinance.Id, payPassword: string) { return realUserRequest<Api.RealFinance.Id, { id: Api.RealFinance.Id; payPassword: string }>({ url: '/finance/orders/redeem', method: 'POST', data: { id, payPassword } }); }
