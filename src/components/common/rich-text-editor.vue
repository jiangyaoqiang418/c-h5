...
function onInput(event: any) {
  currentHtml = sanitizeRichText(event.detail?.html || '');
...
async function setLink() {
  ...
  if (!/^https?:\/\//i.test(href)) return uni.showToast({ title: '链接必须以 http:// 或 https:// 开头', icon: 'none' });
...
<template>
  <view class="rich-editor" :class="{ disabled }">
    <view class="toolbar">
      <button type="default" size="mini" @click="format('bold')">加粗</button>
      <button type="default" size="mini" @click="format('italic')">斜体</button>
      <button type="default" size="mini" @click="format('underline')">下划线</button>
      <button type="default" size="mini" @click="format('header', 'H2')">标题</button>
      <button type="default" size="mini" @click="format('list', 'bullet')">列表</button>
      <button type="default" size="mini" @click="format('blockquote')">引用</button>
      <button type="default" size="mini" @click="setLink">链接</button>
      <button type="default" size="mini" :loading="uploading" @click="insertImage">图片</button>
      <button type="default" size="mini" @click="editorContext?.undo()">撤销</button>
      <button type="default" size="mini" @click="editorContext?.redo()">重做</button>
    </view>
    ...
    <text class="tip">支持标题、列表和图片，图片会先上传后插入，最多 30 张。</text>
  </view>
</template>
...
.toolbar { display: flex; flex-wrap: wrap; gap: 8rpx; padding: 12rpx; border-bottom: 1rpx solid #e5e6eb; background: #f7f8fa; }
.toolbar button { margin: 0; font-size: 22rpx; line-height: 48rpx; }
.editor { min-height: 320rpx; padding: 20rpx; font-size: 26rpx; line-height: 1.7; }
