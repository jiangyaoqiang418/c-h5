...
export async function createProductWithReceipt(params: Api.RealProduct.ProductCreateParams, images: Api.RealProduct.FileUploadResult[], stillActive: () => boolean) {
  ...
  try {
    ...
    const categories = enabledThirdLevelCategories(await fetchCategoryTree({ onlyEnabled: true, onlyWithProduct: false }));
    if (!current()) return;
    if (!categories.some(item => item.id === String(request.categoryId))) throw new Error('所选分类已不可用，请重新选择分类');
